...
void Gra::nowagra() {
    ...
    etap = 1;
    emit zmianaetapu("<br>RUCH<br><b>BIALYCH<br></b>");
    emit saveOn();
...
void Gra::wczytajgre() {
    ...
    if(!fin.fail()){
        ...
        if(etap==1)
            emit zmianaetapu("<br>RUCH<br><b>BIALYCH<br></b>");
        if(etap==2)
            emit zmianaetapu("<br>RUCH<br><b>CZARNYCH<br></b>");
        emit saveOn();
...
void Gra::sprawdzMBpionek(int kolor) {
...
void Gra::sprawdzMRpole(int x, int y, int pionek) {

    wyzerujMRpole();

    switch(pionek){
    case 1: {
            if (planszagry[x-1][y-1]==0)
                MRpole[x-3][y-3] = 1;
            if (planszagry[x-1][y+1]==0)
                MRpole[x-3][y-1] = 1;
        } break;
    ...
    case 11: {
            for (temp=0; temp<7; temp++){
                if(planszagry[x+1+temp][y-1-temp]==0)
                    MRpole[x-1+temp][y-3-temp] = 1;
                else
                    break;
            }
            for (temp=0; temp<7; temp++){
                if(planszagry[x+1+temp][y+1+temp]==0)
                    MRpole[x-1+temp][y-1+temp] = 1;
                else
                    break;
            }
            for (temp=0; temp<7; temp++){
                if(planszagry[x-1-temp][y-1-temp]==0)
                    MRpole[x-3-temp][y-3-temp] = 1;
                else
                    break;
            }
            for (temp=0; temp<7; temp++){
                if(planszagry[x-1-temp][y+1+temp]==0)
                    MRpole[x-3-temp][y-1+temp] = 1;
                else
                    break;
            }
        } break;
